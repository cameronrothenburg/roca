...
fn build_combined_registry(files: &HashMap<PathBuf, SourceFile>) -> ContractRegistry {
    // Start with an empty file to get stdlib loaded
    let empty = crate::parse::parse("");
    let mut registry = ContractRegistry::build(&empty);

    // Then load all user files into the same registry
