...
impl<'a, 'b: 'a, 'c> Body<'a, 'b, 'c> {
    /// Call with crash handler — retry loop + error chain dispatch.
    fn emit_crash_call(
        &mut self,
        func_ref: ir::FuncRef,
        args: &[Value],
    ...
    /// Emit crash handler chain (ok/err branch + chain steps).
    ...
    /// Emit crash chain steps (log, halt, panic, skip, fallback).
