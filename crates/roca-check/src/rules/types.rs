...
use crate::rule::Rule;
use crate::context::{FnCheckContext, ItemContext, StmtContext, ExprContext};
use crate::walker::{resolve_type, infer_type_with_registry, type_ref_to_name};
        ...
        match ctx.expr {
            ...
            Expr::Call { target, args } => {
                if let Expr::Ident(fn_name) = target.as_ref() {
                    ...
                    if !found {
                        if let Some(resolved) = roca_resolve::find_imported_fn(fn_name, ctx.check.file, ctx.check.source_dir.as_deref()) {
