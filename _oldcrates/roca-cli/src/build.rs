...
pub fn build_file(path: &Path) {
    ...
    let native_result = roca_native::test_runner::run_tests(&file);
...
pub fn build_directory(dir: &Path) {
    ...
    for (_file_path, fp_str, file) in &checked {
        let result = roca_native::test_runner::run_tests(file);
...
fn write_package_json(
    project_dir: &Path,
    out_dir: &Path,
    checked: &[(std::path::PathBuf, String, roca_ast::SourceFile)],
) {
    use roca_ast::Item;
