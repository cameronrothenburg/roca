...
fn full_user_registration() {
    assert_eq!(run(
        ...
        "#,
        r#"
            // Note: Email.validate returns [Email, null] tuple
            // but User.validate uses it directly — crash halt would throw on error
            // For this test, we call with valid data
            const [emailResult, _e0] = Email.validate("cam@test.com");
            const [user, _e1] = User.validate("cam", "cam@test.com");

            console.log(user.name);
            console.log(user.to_string());

            // Test error path
            const [bad, err] = User.validate("", "a@b.com");
            console.log(bad);
...
fn error_checked_with_truthiness() {
    ...
    assert_eq!(run(
        ...
        "#,
        r#"
            const [v1, e1] = validate("hello");
            console.log(e1 ? "error" : "ok");

            const [v2, e2] = validate("");
            console.log(e2 ? "error" : "ok");
            console.log(e2.message);

            const [v3, e3] = validate("bad");
