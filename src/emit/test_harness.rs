...
/// Emit a self-contained test harness as JS.
/// Embeds the main code + test assertions in one file.
/// Returns None if no test blocks exist.
pub fn emit_tests(file: &roca::SourceFile) -> Option<String> {
    ...
    // Embed the main code (without exports)
    let main_js = super::emit(file).replace("export ", "");
    // Emit as raw eval — simpler than reconstructing the AST
    let main_code = ast.str(&main_js);
    let mut eval_args = ast.vec();
    eval_args.push(Argument::from(ast.expression_string_literal(SPAN, main_code, None)));

    // Actually, just use a trick: write the main code followed by test code
    // We'll build the test assertions in OXC and prepend the main code as a string
        ...
        match item {
            roca::Item::Function(f) => {
                if let Some(test) = &f.test {
                    emit_fn_tests(&ast, &f.name, f.returns_err, test, &mut body);
                }
            }
            roca::Item::Struct(s) => {
                for method in &s.methods {
                    if let Some(test) = &method.test {
                        emit_static_method_tests(&ast, &s.name, &method.name, method.returns_err, test, &mut body);
                    }
                }
            }
            _ => {}
    ...
    // Summary: console.log(_passed + " passed, " + _failed + " failed")
    ...
    // Prepend main code (without exports) before the test assertions
    Some(format!("{}\n{}", main_js, test_code))
}
    ...
    for (i, case) in test.cases.iter().enumerate() {
        ...
        match case {
            roca::TestCase::Equals { args, expected } => {
                ...
                let result = if returns_err {
                    index_access(ast, call, 0)
                } else {
                    call
                };
...
fn emit_assert_eq<'a>(
    ...
    // else: _failed++; console.log("FAIL: label")
    ...
    let fail_msg_str = format!("FAIL: {}", label);
    let fail_msg = ast.str(&fail_msg_str);
