...
pub fn run_repl() {
    ...
    loop {
        ...
        if is_definition(&input) {
            ...
            src.push('\n');
            src.push_str(&input);
            match roca_parse::try_parse(&src) {
                Ok(file) => {
                    let errors = roca_check::check(&file);
...
fn eval_expr(input: &str, defs: &[String]) {
    ...
    if let Ok(file) = roca_parse::try_parse(&expr_src) {
        let errors = roca_check::check(&file);
        ...
        let emitted = roca_js::emit(&file).lines()
    ...
    match roca_parse::try_parse(&stmt_src) {
        Ok(file) => {
            ...
            let js = roca_js::emit(&file).lines()
                .map(|l| l.strip_prefix("export ").unwrap_or(l))
                .collect::<Vec<_>>()
                .join("\n");
            run_bun(&format!("{}\n__repl__();", js));
...
pub fn run_repl_native() {
    ...
    loop {
        ...
        match line.as_str() {
            ...
            ":clear" | ":c" => { defs.clear(); context.clear(); println!("cleared"); continue; }
            ":mem" => {
                let (a, f, r, rel, live) = roca_native::runtime::MEM.stats();
                println!("allocs={} frees={} retains={} releases={} live_bytes={}", a, f, r, rel, live);
                continue;
            }
            ":mem reset" => {
                roca_native::runtime::MEM.reset();
                println!("memory counters reset");
                continue;
            }
            ":debug on" => { roca_native::runtime::MEM.set_debug(true); println!("memory debug on"); continue; }
            ":debug off" => { roca_native::runtime::MEM.set_debug(false); println!("memory debug off"); continue; }
            _ => {}
        }
...
fn eval_expr_native(input: &str, defs: &[String]) {
    ...
    for (ret_type, is_string) in &[("Number", false), ("String", true)] {
        ...
        let file = match roca_parse::try_parse(&expr_src) {
            Ok(f) => f,
            Err(_) => continue,
        };
