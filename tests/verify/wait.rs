...
fn wait_calls_appear_in_crash() {
    // Verify the checker sees wait calls to err-returning functions and requires crash handlers
    let file = roca::parse::parse(r#"
    ...
    assert!(errors.iter().any(|e| e.code == "missing-crash"),
        "wait calls to err-returning functions should require crash handlers, got: {:?}", errors);
}
