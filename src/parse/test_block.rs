...
impl Parser {
    /// Parse: test { cases }
    pub fn parse_test_block(&mut self) -> TestBlock {
    ...
    fn parse_test_case(&mut self) -> TestCase {
                ...
                match self.peek().clone() {
                    Token::NumberLit(n) => {
                        ...
                        if self.at(&Token::LBrace) {
                            ...
                            self.expect(&Token::RBrace);
                            return TestCase::StatusMock { status, mocks };
                        }
        ...
        if self.eat(&Token::Is) {
            ...
            if self.at(&Token::Ok) {
                ...
                panic!("expected Ok or err after 'is', got {:?}", self.peek());
            }
            ...
            panic!("expected == or is after test self(), got {:?}", self.peek());
        }
    }

    fn parse_test_args(&mut self) -> Vec<Expr> {
    ...
    fn parse_test_mocks(&mut self) -> Vec<TestMock> {
