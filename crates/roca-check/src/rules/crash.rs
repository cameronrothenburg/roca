...
fn build_scope(f: &FnDef, registry: &roca_resolve::ContractRegistry) -> std::collections::HashMap<String, String> {
...
fn collect_scope_from_stmts(
    ...
    registry: &roca_resolve::ContractRegistry,
) {
    for stmt in stmts {
        match stmt {
...
fn collect_calls_in_expr(expr: &Expr, calls: &mut Vec<String>) {
    match expr {
        ...
        Expr::Match { value, arms } => {
            collect_calls_in_expr(value, calls);
            for arm in arms {
                if let Some(roca_ast::MatchPattern::Value(p)) = &arm.pattern { collect_calls_in_expr(p, calls); }
